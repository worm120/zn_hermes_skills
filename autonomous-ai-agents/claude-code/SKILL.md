...
## Reading PDFs with Images/Charts

Claude Code uses multimodal models (Claude Sonnet/Opus) that can see images, charts, and diagrams. When you need Claude to analyze a PDF with visual content (charts, screenshots, scanned slides):

### Correct approach — let Claude see the images

**DO NOT** ask Claude to extract text from the PDF (e.g., "use pymupdf to read this PDF"). Claude's `Read` tool handles PDF files natively and can see embedded images. Just pass the file path:

```terminal(command="claude -p 'Read this PDF and analyze the charts and diagrams: /path/to/file.pdf' --model sonnet --allowedTools 'Read' --max-turns 20", timeout=180)
```

### Size limit workaround (>100MB PDFs)

Claude Code's `Read` tool has a **~100MB file size limit**. For larger PDFs:

1. **Split** the PDF into smaller chunks using pymupdf:
   ```
   python3 -c "
   import pymupdf
   ...
   new.save('/tmp/sample.pdf')
   "
   ```

2. **Render pages to PNG** (Claude needs PNG/JPEG to see images when Read can't handle the file):
   ```
   mkdir -p /tmp/pages
   python3 -c "
   import pymupdf
...
3. **Feed PNGs to Claude** — use `--dangerously-skip-permissions` when files are in `/tmp/` or other non-project directories:
   ```terminal(command="claude --dangerously-skip-permissions -p 'Look at all PNG files in /tmp/pages/ and analyze the charts' --allowedTools 'Read' --max-turns 20", timeout=300)
   ```

**Why this matters**: Claude is multimodal. Asking it to extract text first (``-p 'use pymupdf to read this PDF'``) defeats the purpose — it sees the charts/diagrams natively when given image files or small PDFs directly. Always prefer the direct file-path approach.

### Batch sizing for PNG analysis (critical)

**Claude Code struggles with large batches of images** — requests with 100+ PNGs frequently get SIGINT'd (exit code 130) or silently timeout. The safe approach:

| Batch size | Reliability | Best for |
|-----------|------------|----------|
| **10-25 pages** | ✅ Highly reliable | Detailed analysis per page |
| 25-50 pages | ⚠️ May be OK with generous timeout | Quick scanning |
| 100+ pages | ❌ Frequent timeouts | Avoid |

**Split large PDFs into multiple Claude calls**, each covering 10-25 pages. This is more reliable and produces better per-chapter analysis than one giant batch:

```terminal(command="claude --dangerously-skip-permissions -p 'Look at /tmp/pages/page_046.png to page_0070.png (25 pages)...' --model sonnet --max-turns 30 --allowedTools 'Read'", timeout=300)
```

**Contact-sheet mode** — for slide-deck PDFs where each page is a compact slide, combine 4-6 pages into one image via `pdf_to_pngs.py --sheets 6`. This reduces Claude Read calls by 4-6x, fitting 300+ pages into one Claude call. Use `--dpi 80` for slide decks, 150 for detailed documents.

**File-naming pitfall**: Use consistent padding — 4-digit for 100+ page PDFs, 3-digit for smaller. Mixed naming (page_046.png vs page_0046.png) causes Claude to miss files silently. Script auto-detects this now.
...
Long `claude -p` sessions (30+ turns, many images) can get **interrupted with SIGINT (exit code 130)** when run in pty mode. Two reliable alternatives:

1. **Background mode** (preferred for long analysis):
   ```terminal(command="claude --dangerously-skip-permissions -p 'task...' --model sonnet --max-turns 30 --allowedTools 'Read'", background=true, notify_on_complete=true, timeout=600)
   ```

2. **Smaller max-turns** (20-30 instead of 100+) when running in foreground pty mode.

### Model selection for cost/speed

| Task complexity | Recommended model |
|----------------|-------------------|
| Heavy analysis (PDF with charts, complex code) | `--model sonnet` (balanced) |
| Deep reasoning (architecture, debugging) | `--model opus` |
| Simple tasks (reviews, summaries) | `--model sonnet` |
| Cheapest (CI, quick tasks) | `--model haiku` |

Use `--model sonnet` as the default for most PDF-reading tasks; switch to `--model opus` only when you need maximum reasoning depth.
...
13. **PDF >100MB cannot be read by Claude's Read tool** — it silently fails. Workaround: split with pymupdf, render pages to PNG, then feed PNGs to Claude (see "Reading PDFs with Images/Charts" section above).
14. **When analyzing PDFs with charts/diagrams, do NOT ask Claude to extract text** — you're wasting its multimodal capability. Just pass the file path and let Claude read it natively. If the file is too large, render to PNG first.
16. **Claude Code needs `pdftoppm` (poppler-utils) to parse PDFs natively** — on systems missing this (e.g., NixOS without the package), rendering PDF pages to PNG via pymupdf is the reliable workaround.
17. **Feeding >25 PNGs to Claude causes SIGINT timeouts** — print mode (`-p`) with 50+ images frequently gets exit code 130. Break into batches of 10-25 pages per call with `--max-turns 20-30` each.
18. **`/tmp/` directory access requires `--dangerously-skip-permissions`** — Claude prompts for permission on first access to system directories. Always add the flag when feeding PNGs from `/tmp/`.
19. **PTY mode with long-running `claude -p` is unreliable** — background mode (`background=true, notify_on_complete=true`) or small `--max-turns` values are more reliable for multi-image analysis.
